...
class AccelCalFile(object):
    ...
    def getHighAccelerometer(self, exclude=None):
        """ Get the high-G accelerometer channel.
            ...
            @keyword exclude: A list of channel IDs to ignore.
        """
        print("\n2) Entered getHighAccelerometer!")
        exclude = exclude or []
        ...
        if ch is not None:
            print("\tFound High Accel Channel")
            return ch
    ...
    def findQuietTimes(cls, data, span, searchOverlap=0.5):
        ...
        """
        print(f"\n5) Entered findQuietTimes with span {span} and DATALENGTH {len(data)}!")
        ...
        print(f"\tBest Start Indices are {bestStartIndices} (indices are from the sectioned data)")
    ...
    def _analyze(self, accelChannel, thres=4, start=5000, startTime=None, length=5000, lengthTime=None,
        ...
        if sampRate < 1000:
            raise CalibrationError(
                f"Channel {accelChannel.id} ({accelChannel.name}) had a low sample rate: {sampRate} Hz",
        ...
        _print(f"\tData has been rotated:\n\t\t{data}")
        ...
        print(f"\nshakeSTARTOG {shakeStartIndex}: {hp_data[shakeStartIndex][0]} time - for finding THRES {thres}."
              f"\nThe value is {hp_data[shakeStartIndex][1]} and before it is {hp_data[shakeStartIndex - 1][1]}\n")
        shakeEndIndex = self.getLastIndex(hp_data, smallShakeThreshold, self.axisIds)

        print(f"\nshakeENDOG {shakeEndIndex}: {hp_data[shakeEndIndex][0]} time - for finding SMALL THRES {smallShakeThreshold}."
              f"\nThe value is {hp_data[shakeEndIndex][1]} and before it is {hp_data[shakeEndIndex - 1][1]}\n")

        offsetCalcSpan = 1.0  # Length of time to grab for offset calculations
        offsetCalcSpan = int(offsetCalcSpan * sampRate)
        print(f"\tWorking to find a quiet region with {offsetCalcSpan} values (OffsetCalcSpan)")
        spanBetweenShakes = int(timeBetweenShakes * sampRate)
        if spanBetweenShakes > offsetCalcSpan:
            print(f"\tNvm, spanBtwnShakes is bigger: {spanBetweenShakes}.")
            ...
            quietStart = 0
            quietEnd = len(
                data)  # support for old files, just search everywhere for some peace and quiet in this noise filled world

        print(f"DATASHAPE RN IS {data.shape}")
        print(f"\tBeginning search with QuietStart Idx = {quietStart}: {data[quietStart][0]}, QuietEnd idx = {quietEnd-1}+1: {data[quietEnd-1][0]}")
        print(f"FINDING QUIET OF {data[quietStart:quietEnd, 1].shape}")
        ...
        print(f"\tConsidering whole data: quietStarts is now {quietStarts} with span {offsetCalcSpan}")

        means = self.getMeans(data, quietStarts, offsetCalcSpan, sampRate, lowpass)
        print(f"\tFirst offsets (means) are found: {means}")
        ...
        print(f"\tShake Region Start Index: {shakeStartIndex + start} because start {start} is added.")
        print(f"\tShake Region Stop Index: {shakeStartIndex + stop} because stop {stop} is added.")

        shakeRegionStart = shakeStartIndex + start
        shakeRegionEnd = shakeStartIndex + stop
        print(f"\tGoing to work with hp_data shake at {hp_data[shakeRegionStart][0:2]} to {hp_data[shakeRegionEnd][0:2]}")
        print(hp_data.shape)
        ...
        hp_data = hp_data[shakeRegionStart:shakeRegionEnd]
        print(f"HPDATA FINAL SIZE: {hp_data.shape}")
        # this does not match up with the hp_data time section. Yikes!
        ...
        cal = XYZ(referenceRMS / rms.x,
                  referenceRMS / rms.y,
                  referenceRMS / rms.z)
        # print(f"{hp_data[shakeRegionStart:shakeRegionEnd, :]} ISX")
        print(f"\tGains found - {cal} using {referenceRMS} with amp. {rms.x}")
...
import idelib
Obj = AccelCalFile("jas_calibrationex\\DAQ11620_000007.IDE")


print("\n++++++++++++++++++++++++++++++++++++7x++++++++++++++++++++++++++++++++++++++")
with idelib.importFile("jas_calibrationex\\DAQ11620_000007.IDE") as ds:
    dev = endaq.device.fromRecording(ds)
    cal = Calibrator(dev=dev)
    calAxes, calAxesLo = Calibrator.getAxisFlips(dev)
    print("LO ACCEL STUFF")
    Calibrator.getGravity(dev, -1.0180516227559055)
    cal.calculateOffset(1.064713653072725, -1.0180516227559055, 1)

    print("HI ACCEL STUFF")
    Calibrator.getGravity(dev, -1.3319126397160685)
    cal.calculateOffset(1.3489956861999413, -1.3319126397160685, 1)

"""
print("+++++++++++++++++++++++++++++++++++6z+++++++++++++++++++++++++++++++++++++++")
with idelib.importFile("jas_calibrationex\\DAQ11620_000006.IDE") as ds:
    dev = endaq.device.fromRecording(ds)
    Calibrator.getAxisFlips(dev)
print("++++++++++++++++++++++++++++++++++++5y++++++++++++++++++++++++++++++++++++++")
with idelib.importFile("jas_calibrationex\\DAQ11620_000005.IDE") as ds:
    dev = endaq.device.fromRecording(ds)
    Calibrator.getAxisFlips(dev)
"""