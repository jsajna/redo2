...
class Calibrator(object):
    ...
    def getGravity(cls, dev, calFiles):
        ...
        print("flips coming into gravity are : ", flips)
        print("flipslo coming into gravity are : ", flipsLo)
        ...
        print(activeFlips, " is flips")
        gravity = [m / abs(m) for m in measurement]
        print("GRAVIT IS ", gravity)
        if gravity[2] != 1:
    ...
    def calculate(self, filenames=None, calAxes=None, calAxesLo=None):
        ...
        if self.hasHiAccel and not self.hasLoAccel:
            ...
            print(f"{folder}_gains = XYZ({self.cal.x}, {self.cal.y}, {self.cal.z})")
            print(f"{folder}_offsets = XYZ({self.offsets.x}, {self.offsets.y}, {self.offsets.z})")

            print(f"(pytest.lazy_fixture('{folder}'), {folder}_gains),")
            print(f"(pytest.lazy_fixture('{folder}'), {folder}_offsets),")
            print(f"(pytest.lazy_fixture('{folder}'), {folder}_gains, {folder}_offsets, {folder}),")
            print(f"(pytest.lazy_fixture('{folder}'))")

            print("RMS HIGH")
            print(
                f"(pytest.lazy_fixture('{folder}'), XYZ({self.calFiles.x.rms.x}, {self.calFiles.y.rms.y}, {self.calFiles.z.rms.z}))")

            print("MEANS HIGH")
            print(
                f"(pytest.lazy_fixture('{folder}'), XYZ({self.calFiles.x.means.x}, {self.calFiles.y.means.y}, {self.calFiles.z.means.z}))")
        ...
        self.offsetsLo = XYZ()
        print("Calibration values Low:")
        for i, axis in enumerate('XYZ'):
            self.offsetsLo[i] = self.calculateOffset(self.calLo[i], self.calFiles[i].meansLo[i], gravity[i])
            print(
                f"  {axis}: cal={self.calLo[i]!r},\tmean={self.calFiles[i].meansLo[i]!r},\toffset={self.offsetsLo[i]}")

        print(f"{folder}_gains = XYZ({self.cal.x}, {self.cal.y}, {self.cal.z})")
        print(f"{folder}_gains_dc = XYZ({self.calLo.x}, {self.calLo.y}, {self.calLo.z})")
        print(f"{folder}_offsets = XYZ({self.offsets.x}, {self.offsets.y}, {self.offsets.z})")
        print(f"{folder}_offsets_dc = XYZ({self.offsetsLo.x}, {self.offsetsLo.y}, {self.offsetsLo.z})")

        print(f"(pytest.lazy_fixture('{folder}'), {folder}_gains),")
        print(f"(pytest.lazy_fixture('{folder}'), {folder}_offsets),")
        print(f"(pytest.lazy_fixture('{folder}'), {folder}_gains_dc),")
        print(f"(pytest.lazy_fixture('{folder}'), {folder}_offsets_dc),")
        print(f"(pytest.lazy_fixture('{folder}'), {folder}_gains, {folder}_offsets, {folder}),")
        print(f"(pytest.lazy_fixture('{folder}'), {folder}_gains_dc, {folder}_offsets_dc, {folder}),")
        print(f"(pytest.lazy_fixture('{folder}'))")

        print("RMS HIGH")
        print(f"(pytest.lazy_fixture('{folder}'), XYZ({self.calFiles.x.rms.x}, {self.calFiles.y.rms.y}, {self.calFiles.z.rms.z}))")
        print("RMS LOW")
        print(f"(pytest.lazy_fixture('{folder}'), XYZ({self.calFiles.x.rmsLo.x}, {self.calFiles.y.rmsLo.y}, {self.calFiles.z.rmsLo.z}))")

        print("MEANS HIGH")
        print(f"(pytest.lazy_fixture('{folder}'), XYZ({self.calFiles.x.means.x}, {self.calFiles.y.means.y}, {self.calFiles.z.means.z}))")
        print("MEANS LOW")
        print(f"(pytest.lazy_fixture('{folder}'), XYZ({self.calFiles.x.meansLo.x}, {self.calFiles.y.meansLo.y}, {self.calFiles.z.meansLo.z}))")
...
print(os.getcwd())
Cal.calculate(filenames=filenames(f"..\\testing\\{folder}"))
