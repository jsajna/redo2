#
#
# Thoroughly hacked-up xmodem library
# Tim Gipson / MIDE Technology Corp.
#
# This version provides some improvements / hacks to workaround an implementation detail
# of certain "serial" devices operating over a USB-serial port. The main one is that
# each xmodem packet must be prepared and sent as a single pyserial write(...) call
# to encourage pyserial / Windows to packetize it as a single USB packet, not split the
# xmodem packet up into multiple USB transfers. The EFM32 bootloader is a bit cheesy and
# requires each xmodem packet to occupy a single logical USB transfer at a known (no) offset.
#
# Don't ask me how this jives with the 133-byte xmodem packet vs. the 64-byte USB payload size,
# but I ass-ume this is handled by the on-chip USB hardware peripheral somehow, reassembling the
# linked transfers into a single e.g. DMA dump.
#
# WARNING: Since I only needed 'send' capability, only the 'send' function has been modified.
#   The 'recv' function will presumably have the same problems and is still broken.
#
# Other tweaks: Remove stupidity around user-supplied "getc/putc" functions to allow this to work
# from inside a class (i.e. where there is a hidden 'self' parameter passed with all function calls,
# leading to "expects exactly n arguments, but got n+1" errors everywhere.)
# Instead we pass in an instance of pyserial port and hand-edit all the getc/putc calls
# to be directly compatible with port.read() and port.write().
...
import sys # to print some dots...don't ask...
...
class XMODEM(object):
    ...
    def __init__(self, port, mode='xmodem', pad='\x1a'):
        def _putc_debug(data):
            #print "Raw data passed into putc (len=%d):" % len(data)
            #print data
            #print "Expanded:"
        ...
        def _getc_debug(nBytes):
            data = port.read(nBytes)
            # print("<<< [", end=' ')
            # print ["0x%02x " % ord(x) for x in data],
            # print "]"
            # if len(data) is not nBytes:
            #     print "WARN: Expected %d bytes, but got %d" % (nBytes, len(data))
            # if(port.inWaiting()):
            #     print "NOTE: %d bytes waiting to be read" % port.inWaiting()
            return data

        self.port = port
        self.getc = port.read
        self.putc = port.write
        #self.getc = _getc_debug
        #self.putc = _putc_debug
        self.mode = mode
        self.pad = pad
    ...
    def send(self, stream, retry=16, timeout=60, quiet=0, callback=None):
        ...
        while True:
            ...
            # emit packet
            while True:
                # HACK HACK HACK: EFM32 bootloader expects entire packet sent in a single USB xfer (wtf???).
                # The unmodified xmodem library sends some parts of the data as single characters, which has the effect of
                # sending several 1-byte USB packets and confusing the device.
                # So modified to bundle them up in 1 buffer and send it all at once.
                tx_packet_temp = bytearray([])
                if packet_size == 128:
                    #self.putc(SOH)
                    tx_packet_temp.append(SOH)
                else:  # packet_size == 1024
                    #self.putc(STX)
                    tx_packet_temp.append(STX)
                #self.putc(chr(sequence))
                tx_packet_temp.append(chr(sequence))
                #self.putc(chr(0xff - sequence))
                tx_packet_temp.append(chr(0xff - sequence))
                #self.putc(data)
                tx_packet_temp.extend(data)
                if crc_mode:
                    #self.putc(chr(crc >> 8))
                    tx_packet_temp.append(chr(crc >> 8))
                    #self.putc(chr(crc & 0xff))
                    tx_packet_temp.append(chr(crc & 0xff))
                else:
                    #self.putc(chr(crc))
                    tx_packet_temp.append(chr(crc))

                self.putc(tx_packet_temp)
                char = self.getc(1)
                if char == ACK:
                    success_count += 1
