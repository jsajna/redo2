...
# import paths  # Just importing should set things
...
#         raise NotImplementedError("Refactor AccelCalFile.__str__()!")
    ...
    def analyze(self):
        ...
        if accelChannel is not None:
            exclude_id = [accelChannel.id]  # we will exclude the current accelChannel from the the search of lows?
        ...
        if accelChannelLo:
            ...
            print(f"MEANS LO FIRST FOUND: {self.meansLo}")
...
class Calibrator(object):
    ...
    def calculate(self, filenames=None, calAxes=None, calAxesLo=None):
        ...
        if filenames is None:
            ...
            print("GOTGERE")
            filenames = self.getFiles("jas_calibrationex")
        ...
        self.offsetsLo = XYZ()
        print("Lo Calibration Values")
        for i, axis in enumerate('XYZ'):
            self.offsetsLo[i] = self.calculateOffset(self.calLo[i], self.calFiles[i].meansLo[i], gravity[i])
            print(f"  {axis}: cal={self.calLo[i]!r},\tmean={self.calFiles[i].meansLo[i]!r},\toffset={self.offsetsLo[i]}")
...
class CalibrationFileManager:
    ...
    def analyze(self, accelChannel):
        self.findShakeSegment(accelChannel)
        # TODO: keep adding

    def findShakeSegment(self, accelChannel, start_time=None, end_time=None, smallThres=3, bigThres=6):
        ...
        print(f"SAMPLE RATE {self.sampleRates[accelChannel.id]}")
        ...
        for i in range(1, min(4, hp_data.shape[1])):
            hp_data[:, i] = self.Filter(hp_data[:, i], highpass, self.sampleRates[accelChannel.id], "high")
        ...
        skipSamples = 2001
        if skipSamples:
            data = data[skipSamples:-skipSamples]
            hp_data = hp_data[skipSamples:-skipSamples]  # Shave off skipSamples points from the beginning and end
            print(f"\t{skipSamples} skip Samples has been shaved off the beginning + end")
        print(f"HPSHAPE {hp_data.shape}")

        subch_section = hp_data[:, self.shakenID]
        print(f"SUBCH SECTION {subch_section.shape}")
        ...
        print(f"STARTERS {shakeStartIndex}: {hp_data[shakeStartIndex][0:2]},  {shakeEndIndex}: {hp_data[shakeEndIndex][0:2]}")
        # print(f"SHAKE TIME_RANGE TUPLE WILL BE: {data[shakeStartIndex][0], data[shakeEndIndex][0]}")

        print(f"BRH {hp_data.shape}")
        hp_data = hp_data[shakeStartIndex:shakeEndIndex]
        amp_rms = self.getRMS(hp_data[:, 1:])
        print(f"SHAKE RMS WILL BE: {amp_rms}")

        return amp_rms, (hp_data[shakeStartIndex][0], hp_data[shakeEndIndex][0])

    def findFlatSegment(self, accelChannel, start_time=None, end_time=None, startidx=0, endidx=-1):
        """
        currently has no return: just print statements to show
        """
        print(f"\n\n FINDING FLAT SEGMENT!!! start_time = {start_time}, end_time = {end_time}")
        a = accelChannel.getSession()
        a.removeMean = False
        data = np.flip(np.rot90(a.arrayRange(start_time, end_time)), axis=0)

        # hardcoded skipSamples again to match up with the initial mean/offset from calibration.py
        skipSamples = 2001
        if skipSamples:
            data = data[skipSamples:-skipSamples]
        print(f"DATASHAPE OG: {data.shape}")
        # data = data[startidx:endidx]
        print(f"DATASHAPE: {data.shape} is adjusted to be between {startidx} & {endidx}")
        print(f"DATA BTWM {startidx} & {endidx} is between {data[0][0]} & {data[-1][0]}")

        #print(f"DATA START TIME = {data[0][0]}, DATA END = {data[-1][0]}")
        if accelChannel.id not in self.sampleRates:
            self.sampleRates[accelChannel.id] = a.getSampleRate()

        if not self.shakenID:
            self.shakenID = self.calcShaken(a.arrayRange)
        print("SHAKEN SUBCH is ", self.shakenID)
        subch_section = data[startidx:endidx, self.shakenID]
        print(f"FINDING QUIET OF {subch_section.shape}")

        # also hardcoded span values to ensure matchup with calibration.py
        span = 4002
        ...
        offsetCalcSpan = int(1*self.sampleRates[accelChannel.id])
        print(f"\tConsidering whole data: quietStarts is now {iQuietStart} with span {offsetCalcSpan}")
        offsetmeans = self.findSubchannelOffset(data, iQuietStart, offsetCalcSpan, self.sampleRates[accelChannel.id])
...
#        print(f"Initial Offsets Means is: {self.findSubchannelOffset(data)}")
    ...
    def getQuietestSection(self, id, data, time, searchOverlap=0.5):
        ...
        print(f"SPAN WILL BE FOR QS {span}")
        minStandardDeviation = None
        bestStartIndex = None
        dataLength = len(data)
    ...
    def findSubchannelGain(self, subchannel, target_amplitude, calAxes):
        ...
        print(f"CALAXES IS {calAxes}")
        print(f"SUBCHANNEL GAIN FOUND: {calAxes[0] * self.refRMS / target_amplitude}")
        return calAxes[0] * self.refRMS / target_amplitude

    def findSubchannelOffset(self, subchannel, start, span, sampRate, lowpass=2.55):
        """ Find the target offset for the segment of subchannel provided to meet the target_offset
        """
        print(subchannel)
        if lowpass:
            # Apply lowpass filter
            subchannel = self.Filter(subchannel[:, 1], lowpass, sampRate, "low")
            # print(f"FILTERED: {subchannel}")
        # no filter, just explicitly remove the mean.
        print(f"THE OFFSETMEANS AREEEE: {subchannel[start:start + span].mean()}")
        return subchannel[start:start+span].mean()

    def compensateGainOffset(self, gain, mean, gravity):
        gm = gain * mean
        return gravity - gm
    ...
    # CURRENTLY UNUSED: NEED TO WORK ON GRAVITY METHOD
    def calculateOffset(self, gain, mean, gravity):
        """
        """
        gm = gain * mean
    ...
    # UNUSED: NEED TO WORK ON AXES FLIPS METHOD
    def getGravity(cls, dev, mean):
        """ Get the gravity directions of the data provided.
        ...
        # On Mini, use the 40 or 8 g accelerometer. Otherwise use the secondary (which is always 40g)
        if pn.startswith(('S1', 'S2')):
            print("ENTERED S1 and S2 stuff--looking to use MeanHi")
            activeFlips = flips.x
            activeMeans = mean
        else:
            # if not calFiles.hasLoAccel:
            #   print("No DC accelerometer found!")
            #  return XYZ([1, 1, 1])
            print("Looking to use meanLo")
            print(f"FLIPSLO: {flipsLo}")
            activeFlips = flipsLo.x
            activeMeans = mean
        measurement = activeFlips * activeMeans
        gravity = measurement / abs(measurement)
        # if gravity[2] != 1:
        #   raise CalibrationError("Got the wrong gravity vector on Z for some dumb reason")
    ...
    def getAxisFlips(cls, dev, calAxes=None, calAxesLo=None):
        """ Get the inverted axes specific to the device type.
        """
        print(f"CAL AXES CAME IN WITH: {calAxes}, CAL AXES LO CAME IN WITH {calAxesLo}")
        # FUTURE: Less hardcoding.
        pn = str(dev.partNumber).upper()
        mcu = str(dev.mcuType).upper()
        # TODO: getting the channel to work with first instead of separating into hi and lo
        ...
        if mcu.startswith('STM32'):
            ...
            calAxes = calAxes or (1, 1, 1)
            calAxesLo = calAxesLo or (1, 1, 1)
        print("calAXES ", calAxes)
        print("cal AXESLO ", calAxesLo)
        return calAxes, calAxesLo

    class CalibrationDeviceLibrary(object):
        """ Library class to hold information of a single channel in a calibrating device
        """
        def __init__(self, dev: endaq.device.Recorder, channel: idelib.dataset.channel):
            self.dev = dev
            self.channel = channel
            self.chSession = channel.getSession()
            self.calAxes = XYZ(None)
            self.gravities = XYZ(None)
            self.mean = None
            self.rmsIDEX = XYZ(None)
            self.rmsIDEY = XYZ(None)
            self.rmsIDEZ = XYZ(None)
            self.shakenID = None
            self.shakeTimes = None
            self.quietTimes = None

        def __str__(self):
            print(f"{self.dev} Channel {self.channel.id} - {self.channel.name}")
            print(f"\tShaken ID is Subchannel {self.shakenID}")
            print(f"\tMean of Shaken Subchannel: {self.mean}")
            print(f"\tCalAxes Coefficients: {self.calAxes}")
            print(f"\tGravity Coefficients: {self.gravities}")
            print(f"\tRMS of IDEX: {self.rmsIDEX}")
            print(f"\tRMS of IDEY: {self.rmsIDEY}")
            print(f"\tRMS of IDEZ: {self.rmsIDEZ}")
            print(f"\tShake Time Range: {self.shakeTimes}")
            print(f"\tQuiet Time Ramge: {self.quietTimes}")


import idelib
with idelib.importFile("jas_calibrationex\\DAQ11620_000006.IDE") as ds:
    dev = endaq.device.fromRecording(ds)

Cal = Calibrator()
Cal.calculate(filenames={"jas_calibrationex\\DAQ11620_000005.IDE", "jas_calibrationex\\DAQ11620_000006.IDE", "jas_calibrationex\\DAQ11620_000007.IDE"})