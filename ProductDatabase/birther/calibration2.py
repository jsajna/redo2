...
#         raise NotImplementedError("Refactor AccelCalFile.__str__()!")
    ...
    def analyze(self):
        ...
        if accelChannel is not None:
            exclude_id = [accelChannel.id]  # we will exclude the current accelChannel from the search of lows?
        ...
        if accelChannelLo:
            ...
            print("initial cals are: ", self.calLo)
...
#             _print("Applying low pass filter... ")
            for i in range(1, min(4, data.shape[1])):
                ...
                fig2 = go.Figure()
                fig2.add_trace(go.Scatter(x=data[:, 0], y=data[:, i], mode='lines', name=f'OG of {i}'))
                fig2.add_trace(go.Scatter(x=data[:, 0], y=filtered, mode='lines', name=f'Lowpass of {i}'))
                # fig2.show()
...
#             _print("Calculating means... ")
    ...
    def _analyze(self, accelChannel, thres=4, start=5000, startTime=None, length=5000, lengthTime=None,
        ...
        if skipSamples:
            print(f"{skipSamples} skip samples are being shaved off")
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=data[:, 0], y=data[:, 1], mode='lines', name=f'OG1 {accelChannel}'))
        fig.add_trace(go.Scatter(x=hp_data[:, 0], y=hp_data[:, 1], mode='lines', name=f'HP1 {accelChannel}'))
        fig.show()
        # _print("getting indices... ")

        # Find the start and end of the shaking
        smallShakeThreshold = 3
        print(f"smallShakeThresh = {smallShakeThreshold}, bigShakeThresh = {thres}")
        ...
        print(f"the Quiet Time is {data[quietStarts[0]][0]} to {data[quietStarts[0]+offsetCalcSpan][0]}")
        ...
        print(f"ShakeRegionStart is {hp_data[shakeRegionStart][0]} and ends at {hp_data[shakeRegionEnd][0]}")
...
class Calibrator(object):
    ...
    def getAxisFlips(cls, dev, calAxes=None, calAxesLo=None):
        ...
        print("calAxesHi, ", XYZ(calAxes))
        print("calAxesLo, ", XYZ(calAxesLo))
        return XYZ(calAxes), XYZ(calAxesLo)

    @classmethod   # how is it that this only gets called once, but getGravity in mine is called 6 times
    ...
    def calculateTrans(self, calFiles, cal, low=False):
        ...
        if low:
            # each of these stored vars are XYZs of all the rms's of all subchannels within the accel ch
    ...
    def calculate(self, filenames=None, calAxes=None, calAxesLo=None):
        ...
        # print("Hi Calibration Values of Channel", self.calFiles[0].accelChannel.id)
        for i, axis in enumerate('XYZ'):
            # print(f"AXIS {axis}, Using cal of {self.cal[i]} with means {self.calFiles[i].means[i]} and grav {gravity[i]}")
        ...
        self.offsetsLo = XYZ()
        # print("Lo Calibration Values of Channel ", self.calFiles[0].accelChannelLo.id)
        for i, axis in enumerate('XYZ'):
